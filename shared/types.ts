/* 文件管理模块 ActionRegistry 依赖（这些是运行期依赖，仅在你 import 本文件时加载；
   useDoc / useUI / useFileStore 在 run() 内动态 import 以避免循环初始化崩溃） */
import type { EditorSettings } from '../src/store/useUI.js'
import { docsApi, assetsApi, compileApi, api, filesApi } from '../src/lib/api.js'
import { toast, downloadText } from '../src/lib/ui.js'
...
export type FileKind = 'doc' | 'image' | 'svg' | 'lottie' | 'snippet' | 'template'
...
export interface FileItem {
  ...
  deletedAt?: number
  pinned?: boolean
  refs?: { docId: string; docTitle: string }[]
...
/* ------------------------------------------------------------------ */
/* 块默认值填充（修复 §0.1 默认值不生效 bug）                            */
/* ------------------------------------------------------------------ */

/**
 * 纯函数，不可变。仅当块「未显式设置」某排版字段时，才用 EditorSettings 的默认值填充。
 * 不递归进入 frame 子块（设计 §0.1）。
 */
export function applyBlockDefaults(block: Block, s: EditorSettings): Block {
  if (block.type === 'frame') return block
  const style: BlockStyle = { ...block.style }
  const t = s.typo
  if (style.fontSize == null) style.fontSize = t.defaultFontSize
  if (style.lineHeight == null) style.lineHeight = t.defaultLineHeight
  if (style.fontFamily == null) style.fontFamily = t.defaultFont
  if (style.fontWeight == null) style.fontWeight = t.defaultFontWeight
  if (style.letterSpacing == null) style.letterSpacing = t.defaultLetterSpacing
  // color：EditorSettings 暂无通用颜色默认值来源，保持块原值（后续可在通用组补 defaultTextColor）
  // textAlign / justify：两端对齐由 typo.defaultJustify 决定
  if (style.textAlign == null && t.defaultJustify) style.textAlign = 'justify'
  return { ...block, style }
}
...
export interface FileAction {
  ...
  icon?: any
  appliesTo: FileKind[]
  run: (item: FileItem, ctx: any) => void | Promise<void>
}

/** 动态获取有循环依赖风险的 store（useDoc/useUI/useFileStore），避免模块顶层循环初始化 */
async function getStores() {
  const [{ useDoc }, { useUI }, { useFileStore }] = await Promise.all([
    import('../src/store/useDoc.js'),
    import('../src/store/useUI.js'),
    import('../src/filemgr/useFileStore.js'),
  ])
